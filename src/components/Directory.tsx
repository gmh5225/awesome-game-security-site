...
export default function Directory({ resources, locale, topicCards, savedOnly = false }: { resources: Resource[]; locale: Locale; topicCards?: ReactNode; savedOnly?: boolean }) {
  ...
  const filterSelect = (key: string, label: string, value: string, all: string, values: string[][]) => <label className="filter-field" key={key}><span>{label}</span><select aria-label={label} value={value} onChange={event => update(key, event.target.value)}><option value="">{all}</option>{values.map(([id, text]) => <option key={id} value={id}>{text}</option>)}</select></label>;
