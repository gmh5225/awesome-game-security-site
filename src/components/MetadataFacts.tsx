...
export default function MetadataFacts({ resource, locale, level, kind }: MetadataFactsProps) {
  ...
  const rows = (facts: Fact[]) => facts.map(fact => <div key={fact.key} className="fact-row"><dt>{fact.label}</dt><dd className={fact.value === d.unknown || fact.value === d.linkUnknown ? 'fact-value-muted' : undefined}>{fact.value}</dd></div>);

  return <div className="detail-facts">
    <h2>{d.resourceDetails}</h2>
    <dl className="facts-overview">{rows(overview)}</dl>
    <section className="facts-group">
      <h3>{d.repositoryStatus}</h3>
      ...
      <dl className="facts-status">{rows(status)}</dl>
    </section>
    <details className="facts-history">
      <summary><span>{d.checkHistory}</span><Icon name="arrow" size={15}/></summary>
      <dl>{rows(history)}</dl>
